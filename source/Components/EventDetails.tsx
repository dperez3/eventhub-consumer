...
export const EventDetails = ({ event }: EventDetailsProps) => {
	...
	const nextMode = mode == 'azure' ? 'see Cloud Event' : 'see raw Azure message';
    ...
    return (
		<TitledPanel title="Last Message Details">
			<Box><Text bold={true}>{"<-"} Press arrow key to {nextMode} {"->"}</Text></Box>
			{mode == 'azure' ? <AzureEventDetails azureEvent={event} /> : <CloudEventDetails azureEvent={event} />}
		</TitledPanel>
    );
}
