...
import { cloudEventsAttrPrefix, translateToCloudEvent } from "../services/cloudEventTranslator.js";
...
export const CloudEventDetails = ({ azureEvent }: CloudEventDetailsProps) => {
	...
	const cloudEventAttributes = azureEvent ? getCloudEventAttributes(azureEvent) : [];
	...
	return (
		<Box flexDirection="column" gap={1}>
			...
			<Box flexDirection={"column"}>
				{
					cloudEvent
						? <Text color={'cyan'} dimColor={true}>{JSON.stringify(cloudEvent.data, null, 2)}</Text>
...
function getCloudEventAttributes(cloudEvent: ReceivedEventData) {
	const props = Object.entries(cloudEvent.properties ?? {}).filter(x => x[0].startsWith(cloudEventsAttrPrefix));
