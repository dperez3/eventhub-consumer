...
import { EventHubConsumerClient, earliestEventPosition, RetryMode, ReceivedEventData } from "@azure/event-hubs";
...
export type ClientOptions = ClientConfiguration & {
    ...
    eventsUpdated: (newEventsMap: Map<string, ReceivedEventData>) => Promise<void>;
};

export async function mainOld({ eventHubs, storage, credential }: ClientOptions): Promise<void> {
    ...
    const consumerClient = new EventHubConsumerClient(eventHubs.consumerGroup, fullyQualifiedNamespace, eventHubs.eventHubName, credential, checkpointStore);
...
export async function mainNew({ eventHubs, storage, credential, eventsUpdated }: ClientOptions): Promise<void> {
    const fullyQualifiedNamespace = `${eventHubs.eventHubsResourceName}.servicebus.windows.net`;
...
function getStorageContainerUrl(storageConfig: ClientOptions['storage']): string {
    if (typeof storageConfig == 'string') {
        return storageConfig;
    }

    const baseUrl = `https://${storageConfig!.storageAccountName}.blob.core.windows.net`;
